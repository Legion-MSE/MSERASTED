...
def range_scan(semiranges, rango1, rango2, save_filee, name_file_, timeout):
    ...
    print("\n")
    salida = input(str(rainbow("[MSERASTED] Escaneo finalizado... Presiona enter para volver al menú")).replace("None", "")).split("-")
    print("\n")
    clear()
    main()
...
def main():
    ...
    rainbow(banner)
    rainbow("                Coded by SAPOREAL #LegionMSE ")
    print()
    print() 
    ...
    print("\n\n")
    opt = input(str(rainbow("                        [MSERASTED] Selecciona una opción: ")).replace("None", ""))
    if opt == "1":
        print()
        ips = input(str(rainbow("[MSERASTED] Escribe las ips a escanear separadas por espacios: ")).replace("None", "")).split()
        if len(ips) == 0:
        ...
        for ip in ips:
            if not is_an_ip(ip):
                ...
                main()
        print()
        opt_2 = input(str(rainbow("[MSERASTED] ¿Deseas guardar los resultados en un txt? s/n: ")).replace("None", "")).lower().strip()
        print()
        sis = ["y","yes","yup","yeah","s","si","sip"]
        if opt_2 in sis:
            scan_save_2 = True
            name_file = input(str(rainbow("[MSERASTED] Introduce el nombre al archivo (result_ip_scan.txt by default): ")).replace("None", "")).strip()
            if name_file == "":
            ...
            scan_save_2 = False
            name_file = False
        timeout = input(str(rainbow("[MSERASTED] Introduce un timeout (3 seconds by default): ")).replace("None", ""))
        if timeout.strip() == "":
            timeout = 3
        else:
        ...
        print()
        rangos = input(str(rainbow("[MSERASTED] Introduce el rango de puertos a escanear (Ej: 25530-25580): ")).replace("None", ""))
        ...
        print()
        ips = input(str(rainbow("[MSERASTED] Introduce el directorio del archivo a escanear (ips.txt by default): ")).replace("None", "")).strip()
        if ips == "":
            file = "ips.txt"
        else:
            file = ips
        ...
        if len(archivo_striped) == 0:
            ...
            print()
            ips = input(str(rainbow("[MSERASTED] ¿Deseas establecer ips para escanear? y/n: ")).replace("None", "")).strip()
            sis = ["y","yes","yup","yeah","s","si","sip"]
            if ips.lower() in sis:
                print()
                versions = input(str(rainbow("[MSERASTED] Escribe las versiones para escanear entre comillas ('spigot 1.8.8' 'paperspigot'): ")).replace("None", "")).strip().split("'")
            ...
            print()
            sis = ["y","yes","yup","yeah","s","si","sip"]
            opt2 = input(str(rainbow("[MSERASTED] Se han detectado ips en el txt ¿Deseas revisar las ips antes del escaneo? s/n: ")).replace("None", "")).strip()
            ...
            print()
            opt_2 = input(str(rainbow("[MSERASTED] ¿Deseas guardar los resultados en un txt? s/n: ")).replace("None", "")).strip()
            print()
            if opt_2 in sis:
                scan_save = True
                name_file = input(str(rainbow("[MSERASTED] Introduce el nombre al archivo (result.txt by default): ")).replace("None", "")).strip()
                print()
                if name_file.strip() == "":
                ...
                name_file = False
            sis = ["y","yes","yup","yeah","s","si","sip"]
            opt3 = input(str(rainbow("[MSERASTED] ¿Te gustaría agregar más ips para escanearlas? s/n: ")).replace("None", "")).strip()
            ...
            if "." in rangos:
                ...
                rango2 = int(rangos[1])
            print()
            starting_scan = input(str(rainbow("[MSERASTED] ¿Deseas iniciar el escaneo? s/n: ")).replace("None", "")).strip()
        ...
        rangos_to_scan = input(str(rainbow("[MSERASTED] Introduce los semirangos a escanear (Ej: 142.44.143.* 66.70.180.*): ")).replace("None", "")).split()
        ...
        for semirangoo in rangos_to_scan:
            if is_a_semirange(semirangoo):
                ...
                main()
        print()
        option_2 = input(str(rainbow("[MSERASTED] ¿Deseas guardar los resultados en un txt? s/n: ")).replace("None", "")).lower().strip()
        print()
        if option_2 in sies:
            scan_save = True
            name_file = input(str(rainbow("[MSERASTED] Introduce el nombre al archivo (result_linkscanner.txt by default): ")).replace("None", "")).strip()
            print()
            if name_file == "":
        ...
        if timeout.strip() == "":
            ...
            if not is_a_timeout(timeout):
                ...
                main()
        print()
        ports_range = input(str(rainbow("[MSERASTED] Introduce un rango de puertos (Ej: 25530-25580): ")).replace("None", ""))
        ...
        print("\n")
        rainbow("                        [MSERASTED] Esa opción no existe...")
        print("\n")
